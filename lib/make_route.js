      ...
      router[supported[k][0]]('rest:' + name, mpath + supported[k][1],
        ...
        async (ctx, next) => {
          ...
          try {
            ...
            finalResult.data = helper('rest:convertIdColumn')(route.model || _.get(route, 'options.idColumn'), finalResult.data)
            if (ctx.auth && ctx.auth.permission) finalResult.data = ctx.auth.permission.filter(finalResult.data)
