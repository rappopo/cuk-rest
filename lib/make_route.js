...
module.exports = function(cuk) {
  ...
  return (file, cuksPkg, pkg, router, appDir) => {
    ...
    if (disabled.indexOf(_name) > -1) {
      helper('core:bootTrace')('|  |  |- Disabled => %s', null, null, _name)
      return
    }
    ...
    _.forOwn(route.method, (v, k) => {
      let key = _.camelCase('rest ' + routePath) + ':' + k,
        keys = key.split(':')
      ...
      router[supported[keys[1]][0]](key, mpath + supported[keys[1]][1],
        helper('http:composeMiddleware')(v.middleware, key, true),
        async (ctx, next) => {
        try {
      ...
      helper('core:bootTrace')('|  |  |- Enabled => %s -> [%s] %s%s', _name, k, pkg.cfg.common.mount, mpath)
    })
  }
