...
module.exports = function (cuk) {
  ...
  const convertIdColumn = require('./convert_id_column')(cuk)
  const supported = {
  ...
  return (file, cuksPkg, pkg, router, appDir, cors) => {
    ...
    _.forOwn(route.method, (v, k) => {
      ...
      let middleware = []
      let gMw = config('rest').globalMiddleware || []
      if (_.isString(gMw)) gMw = helper('core:makeChoices')(gMw)
      ...
      if (middleware.indexOf('http:cors') > -1 && cors.indexOf(mpath + supported[k][1]) === -1) {
      ...
      router[supported[k][0]](name, mpath + supported[k][1],
        ...
        async (ctx, next) => {
          ...
          try {
            ...
            if (route.model) finalResult.data = convertIdColumn(route.model, finalResult.data)
